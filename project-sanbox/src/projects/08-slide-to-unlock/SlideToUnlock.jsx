...
export default function SlideToUnlock() {
    ...
    const [showLockSlider, serShowLockSlider] = useState(true);

    const [lockSliderValue, setLockSliderValue] = useState(0);

    const handleLockSliderInout=(e)=>{
        setLockSliderValue(e.target.value);
        // serShowLockSlider(false);
    };
    
    const lockScreen =()=>{
        setLockSliderValue(0);
        serShowLockSlider(true);
        setUiProps({
            uiText: "Unlock screen",
            uiColor: "#eee",
            uiBg: `url(${LockScreenImg}) center/cover no-repeat`
  ...
  return (
    ...
    >
        ...
        {showLockSlider ? (
            <LockSlider 
                width={"250px"} 
                handleInput={handleLockSliderInout} 
